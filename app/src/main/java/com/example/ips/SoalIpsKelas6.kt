...
class SoalIpsKelas6 : Fragment() {

    private var jawabanBenar6 = 0
    private val soalIpssd6: List<List<Pair<String, Int>>> = listOf(
        listOf(
...
""", 0)),
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_soal_ips_kelas6, container, false)
        scoreManagerIps = ScoreManagerIps(requireActivity().applicationContext)
        jawabanBenar6 = if(scoreManagerIps.scoreIpsKelas6 < 1) 0 else scoreManagerIps.scoreIpsKelas6 / 5
        ...
        for(i in 0 until (listRadiogrup.size)){
            val getData = soalIpssd6[jawabanBenar6][i]
            ...
            if(energy.energy == 0){
                ...
                if (grup1.checkedRadioButtonId != -1 && grup2.checkedRadioButtonId != -1 && grup3.checkedRadioButtonId != -1) {
                    if (done) {
                        ...
                        var scoreIpsSd = scoreManagerIps.scoreIpsKelas6
                        if (pilihGrup1 == ch1 && pilihGrup2 == ch2 && pilihGrup3 == ch3) {
                            jawabanBenar6++

                            if (jawabanBenar6 == soalIpssd6.size) {
                                scoreIpsSd += 5
                                scoreManagerIps.scoreIpsKelas6 = scoreIpsSd
                                Toast.makeText(
                                    activity,
